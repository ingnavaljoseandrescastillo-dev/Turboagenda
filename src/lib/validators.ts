...
export const AppointmentSchema = z.object({
  ...
  client_name: z.string().trim().min(2, 'Nome deve ter pelo menos 2 caracteres').max(120),
  client_email: z.string().trim().email('Email invalido').max(254).optional().or(z.literal('')),
  client_phone: z.string().trim().max(25).optional(),
