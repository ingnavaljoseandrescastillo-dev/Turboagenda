...
export function AppointmentCard({ appointment, showActions }: AppointmentCardProps) {
  ...
  async function updateStatus(status: 'confirmed' | 'cancelled') {
    ...
    try {
      ...
      if (!res.ok) {
        console.error('[AppointmentCard] status update failed', json.error)
        throw new Error(json.error ?? 'Nao foi possivel atualizar o agendamento.')
      }
      router.refresh()
    } catch (err) {
    ...
    <div className="p-4 hover:bg-zinc-800/30 transition">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-zinc-400 truncate">{appointment.service?.name ?? 'Servico'}</div>
        ...
        <div className="flex-shrink-0">
          {showActions && appointment.status === 'pending' && (
            <div className="flex gap-1.5">
              <button
                ...
                className="flex items-center gap-1 px-2 py-1.5 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 rounded-lg text-xs font-semibold disabled:opacity-50"
              >
                {loadingStatus === 'confirmed' ? '...' : 'OK'}
              </button>
              <button
                type="button"
                disabled={!!loadingStatus}
                onClick={() => updateStatus('cancelled')}
                className="flex items-center gap-1 px-2 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg text-xs font-semibold disabled:opacity-50"
              >
                {loadingStatus === 'cancelled' ? '...' : 'X'}
              </button>
            </div>
          )}
          {appointment.status === 'completed' && (
            <span className="px-2.5 py-1.5 bg-emerald-500/10 text-emerald-400 rounded-lg text-xs font-semibold">
              OK
            </span>
          )}
          {appointment.status === 'cancelled' && (
            <span className="px-2.5 py-1.5 bg-zinc-800 text-zinc-500 rounded-lg text-xs font-semibold">
              Cancelada
            </span>
          )}
          ...
          {appointment.status === 'confirmed' && (
            <span className="px-2.5 py-1.5 bg-blue-500/10 text-blue-400 rounded-lg text-xs font-semibold">
              Confirmado
            </span>
          )}
        </div>
