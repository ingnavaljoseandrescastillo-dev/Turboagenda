...
export default async function AppointmentsPage() {
  ...
  if (!ownerData) return <p className="text-zinc-500">Negócio não encontrado.</p>
    ...
    <div className="space-y-5 max-w-3xl">
      ...
      {Object.keys(grouped).length === 0 ? (
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-10 text-center">
          ...
          <p className="text-zinc-500 text-sm">Ainda não há agendamentos</p>
        </div>
      ) : (
        Object.entries(grouped).map(([date, appts]) => (
          <div key={date}>
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl overflow-hidden divide-y divide-zinc-800">
              {appts.map((a) => <AppointmentCard key={a.id} appointment={a} showActions />)}
            </div>
          </div>
        ))
      )}
